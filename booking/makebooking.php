...
<script>
  ...
  <!-- client notification: client side-->
...
<center>
<br><br><br>
<img src="logo.png" alt="Finsmarts Consultancy">
<h1 style="font-family: 'Indie Flower', cursive; font-size: 35px;">Booking Page</h1><br>

<table border="1" cellpadding="5" width="800">
	<tr>
		<td valign="top">
		<form action="book.php" method="post" style="padding: 10px;">
			<center><h3 style="font-family: 'Indie Flower', cursive;">Make booking</h3></center>
			<p>Consultant: <input checked = "checked" name="item" type="radio" value="Andy" />Andy
			...
			<table style="width: 70%">
				<tr>
					<td> ClientName:</td>
					<td> <input maxlength="50" name="name" required="" type="text" value="<?php echo $_SESSION["first_name"]; ?>"/></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
				</tr>
				<tr>
			...
			<strong><input style="font-family: 'Indie Flower', cursive;" name="book" type="submit" value="Book" /></strong>
		</form>
		</td>
		<td valign="top">
		
		<form action="delete.php" method="post" style="padding: 10px;">
		<center><h3 style="font-family: 'Indie Flower', cursive;">Cancel booking</h3>
			<p>(Input id written in appointment calendar below)</p></center>
			...
			<strong><p><input style="font-family: 'Indie Flower', cursive; fo" name="cancel" type="submit" value="Cancel" /></p></strong>
		</form>
		</td>
	</tr>
</table>
		...
		$calendar.= '<td class="calendar-day">';
    		...
    		if (mysqli_num_rows($result) > 0) {
    			...
    			while($row = mysqli_fetch_assoc($result)) {
    				...
    				$calendar .= "<b>" . $row["item"] . "</b><br><b>ID:</b> " . $row["id"] . "<br><b>Client:</b> " . $row["name"] . "<br>";
    				if($current_epoch == $row["start_day"] AND $current_epoch != $row["end_day"]) {
    					$calendar .= "<b>Start Time:</b> " . sprintf("%02d:%02d", $row["start_time"]/60/60, ($row["start_time"]%(60*60)/60)) . "<br><hr><br>";
    				}
    				if($current_epoch == $row["start_day"] AND $current_epoch == $row["end_day"]) {
    					$calendar .= "<b>Start Time:</b> " . sprintf("%02d:%02d", $row["start_time"]/60/60, ($row["start_time"]%(60*60)/60)) . "<br>";
    				}
    				if($current_epoch == $row["end_day"]) {
    					$calendar .= "<b>End Time:</b> " . sprintf("%02d:%02d", $row["end_time"]/60/60, ($row["end_time"]%(60*60)/60)) . "<br><hr><br>";
    				}
    				if($current_epoch != $row["start_day"] AND $current_epoch != $row["end_day"]) {
