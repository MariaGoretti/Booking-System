...
if(empty($_SESSION['captcha'] ) ||
	...
	if(empty($errors))
	{
		...
		$sql = "SELECT * FROM $tablename WHERE item='$item' AND (start_day>=$start_day OR end_day>=$start_day)";
			...
			while($row = mysqli_fetch_assoc($result)) {
				...
				for ($i = $start_epoch; $i <= $end_epoch; $i=$i+600) {
					if ($i>($row["start_day"]+$row["start_time"]) && $i<($row["end_day"]+$row["end_time"])) {
						echo '<h3><font color="red">Unfortunately an appointment with the consultant has already been booked for the time requested. Try Again</font></h3>';
						echo "<script>setTimeout(\"location.href = 'makebooking.php';\",1500);</script>";
						goto end;
					}
				}
			}				
		...
		if (mysqli_query($conn, $sql)) {
		    //echo "<h3>Booking succeed. Wait for confirmation</h3>";
			echo "<script type='text/javascript'>alert('Booking successful. Allow notifications to receive your appointment updates.')</script>";
            
			//echo "<script type='text/javascript'>alert('Booking successful.\n Allow notifications to receive your appointment updates.')</script>";
echo"<script>
if(window.Notification && Notification.permission !== 'denied') {
	Notification.requestPermission(function(status) {  // status is 'granted', if accepted by user
		var n = new Notification('Your Appointment', { 
			body: 'You will now receive notification updates about your appointment.',
			icon: '/path/to/icon.png'
		}); 
	});
}
</script>";	
echo "<script>setTimeout(\"location.href = 'makebooking.php';\",3000);</script>";		
		} else {
...
<!--<a href="makebooking.php"><p>Back to the booking page</p></a>-->
